...
def time_music():
    ...
    # time status convert
    convert_time_info_music = time.strftime('%M:%S', time.gmtime(current_time))
    # Get song length by mutagem
    current_song = box_player.curselection()
    song = box_player.get(current_song)
    # adiciona ao diretorio
    song = f'C:/Users/Code/Documents/GitHub/mp3Player/music/{song}.mp3'
    song_mut = MP3(song)
    song_length = song_mut.info.length
    # Convert to minutes
    convert_len = time.strftime('%M:%S', time.gmtime(song_length))
    # Adiciona texto no status bar
    status_bar.config(
        text=f'Time Music: {convert_time_info_music} of {convert_len} ')
    # up time
...
#  Loop Many song
    for song in songs:
        song = song.replace(
            "C:/Users/Code/Documents/GitHub/mp3Player/music/", "")
        song = song.replace(".mp3", "")
...
box_player = Listbox(root, bg="#15F244", fg="#15F244", highlightbackground='#080808',
                     highlightcolor='#080808', background='#080808', width=200, selectbackground='#080808')
...
back_button = Button(controls_frame, image=back_button_img,
                     borderwidth=0, command=back_song)
...
menu_del_song.add_command(
    label="Delete a Song from Playlist", command=delete_song)
menu_del_song.add_command(
    label="Delete ALL Song from Playlist", command=delete_all_songs)
